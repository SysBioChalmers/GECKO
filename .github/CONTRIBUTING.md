...
* As soon as the branch is merged, confirm that `devel` is still possible to merge to `main` (this can be checked [here](https://github.com/SysBioChalmers/GECKO/compare/devel)). If conflicts appear, fix the conflict _locally_ as soon as possible in `devel` and then push it (note, **DO NOT** pull any other changes from `main` to `devel`, just the single file that is creating the conflict).

### Releasing a new version

* A merge of `devel` with `main` invokes a new release.
...
1. Create a pull request from `devel` to `main`:
