...
% --- Fields consumed by writeDiagnosticReport (added for compatibility) ---
nSrcPlus = numel(kcatSources) + 1;                       % per-source rows + unlabelled
diagnostics.changeMagnitudeQ25 = zeros(nSrcPlus, maxGenerations);  % |log(kcat/kcat0)| quartiles
diagnostics.changeMagnitudeQ50 = zeros(nSrcPlus, maxGenerations);  % (natural log; exp(x)=fold change)
diagnostics.changeMagnitudeQ75 = zeros(nSrcPlus, maxGenerations);
diagnostics.proposalAcceptRate = zeros(1, maxGenerations);         % fraction of NEW proposals beating epsilon
diagnostics.boundaryViolationsTrace = zeros(1, maxGenerations);    % fraction of proposals clipped at bio bounds
diagnostics.directednessTrace = ones(1, maxGenerations);           % net displacement / total path (1 = directed)
...
%% Trajectory accumulators (for directedness diagnostic)
prevLogKcat = log(kcat0);          % log-kcat at previous stored generation
cumLogPath  = zeros(D, 1);         % running sum of |per-generation log steps|
...
while rmse > rmseThreshold
    if generation <= maxGenerations
        ...
        if generation == 1
            ...
            boundaryFrac = 0;   % bounds not enforced during prior draw
        else
            ...
            [randomKcats, boundaryFrac] = proposeSimple(parents, sigmaProp_log, ecModel.ec.kcat);
        end
        ...
        % Acceptance rate among NEW proposals only (exploration health signal).
        % Uses the percentile threshold before the minKeep/maxKeep clamp below.
        propAccRate = sum(newRmse <= epsilon) / max(numel(newRmse), 1);
        ...
        % Acceptance / boundary diagnostics for this generation
        diagnostics.proposalAcceptRate(generation) = propAccRate;
        diagnostics.boundaryViolationsTrace(generation) = boundaryFrac;

        % Directedness: how efficiently parameters moved toward their final value.
        % directedness = net displacement / total path travelled (per moved param).
        % Near 1 = purposeful; near 0 with non-trivial steps = random walk.
        stepChange = abs(log(kcats) - prevLogKcat);
        cumLogPath = cumLogPath + stepChange;
        netDisp    = abs(log(kcats) - log(kcat0));
        movedMask  = netDisp > 0.01;                        % moved at least ~1%
        if any(movedMask)
            diagnostics.directednessTrace(generation) = ...
                mean(netDisp(movedMask) ./ max(cumLogPath(movedMask), eps));
        else
            diagnostics.directednessTrace(generation) = 1;  % nothing moved => no wandering
        end
        prevLogKcat = log(kcats);
...
%% Finalize
% Trim all per-generation traces to the number of generations actually stored.
% writeDiagnosticReport indexes some traces with 'end', so trailing
% preallocated zeros must be removed or they corrupt late-stage statistics.
nFilled = max(generation - 1, 1);
perGenFields = {'shrinkageTrace','acceptanceRateTrace','epsilonTrace', ...
    'nSamplesTrace','nAcceptedTrace','activeBySource','nearPriorBySource', ...
    'meanDeviationBySource','varianceRatioBySource','sparsityCountTrace', ...
    'sparsityFractionTrace','diversityTrace','meanAcceptedRMSE', ...
    'medianAcceptedRMSE','proposalWidthTrace','changeMagnitudeQ25', ...
    'changeMagnitudeQ50','changeMagnitudeQ75','proposalAcceptRate', ...
    'boundaryViolationsTrace','directednessTrace'};
for f = 1:numel(perGenFields)
    fld = perGenFields{f};
    if isfield(diagnostics, fld)
        M = diagnostics.(fld);
        diagnostics.(fld) = M(:, 1:min(nFilled, size(M,2)));
    end
end
...
function [proposals, fracClipped] = proposeSimple(parents, sigmaProp_log, priorKcat)
...
%   fracClipped   scalar fraction of proposed values pushed onto a bound
    ...
    % Per-parameter biological bounds (same rule as original, vectorised)
    isExceptional = priorKcat > 1e4;          % e.g. catalase
    loBound = 1e-2 * ones(D, 1);
    hiBound = 1e4  * ones(D, 1);
    loBound(isExceptional) = priorKcat(isExceptional) / 100;
    hiBound(isExceptional) = 1e8;
    
    % Count how many proposed values fall outside bounds (exploration vs wall)
    clipped = (proposals < loBound) | (proposals > hiBound);
    fracClipped = sum(clipped(:)) / numel(proposals);
    
    % Apply biological bounds
    proposals = min(max(proposals, loBound), hiBound);
end
...
function diagnostics = storeSourceMetrics(diagnostics, gen, shrinkWeight, ...
    ...
    for i = 1:numel(kcatSources)
        ...
        if any(idx)
            ...
            % Change-magnitude quartiles, |log(kcat/kcat0)| (exp(x) = fold change)
            cm = abs(log(kcats(idx)) - log(kcat0(idx)));
            diagnostics.changeMagnitudeQ25(i, gen) = prctile(cm, 25);
            diagnostics.changeMagnitudeQ50(i, gen) = prctile(cm, 50);
            diagnostics.changeMagnitudeQ75(i, gen) = prctile(cm, 75);
        end
    end
    ...
    if ~isempty(noKcatSource)
        cm = abs(log(kcats(noKcatSource)) - log(kcat0(noKcatSource)));
        diagnostics.changeMagnitudeQ25(otherIdx, gen) = prctile(cm, 25);
        diagnostics.changeMagnitudeQ50(otherIdx, gen) = prctile(cm, 50);
        diagnostics.changeMagnitudeQ75(otherIdx, gen) = prctile(cm, 75);
    end
end
