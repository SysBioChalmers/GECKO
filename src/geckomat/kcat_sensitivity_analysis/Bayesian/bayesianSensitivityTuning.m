...
%     run. Not compatible with ecModels generated by earlier GECKO versions
%     (pre 3.0).
%
% Name-Value Arguments
% --------------------
% kcatSigmaLog : double
%     vector (same length as ecModel.ec.kcat) with the initial log-space
%     standard deviations (sigmaLog) for kcats. By default, kcatSigmaLog is
%     set from params.bayesian.initSigmaLogDefault and optionally overridden
%     per source by params.bayesian.initSigmaLog (a vector of sigmaLog values
%     aligned with params.bayesian.kcatSources). A custom kcatSigmaLog may
%     also be provided.
...
% -------
% ecModel : struct
%     ecModel with best kcat values applied.
% rmseTrace : double
%     history of best RMSE per generation.
% kcatTrace : double
%     history of posterior kcat means per generation.
...
% prunedModel : struct
%     optional ecModel where insignificant parameter changes are reverted to
%     prior values; empty when pruning is disabled.
%
% Notes
% -----
...
%     ecModel = bayesianSensitivityTuning(ecModel, 'modelAdapter', adapter);
%     ecModel = bayesianSensitivityTuning(ecModel, kcatSigmaLog, modelAdapter);
%
% See also
% --------
