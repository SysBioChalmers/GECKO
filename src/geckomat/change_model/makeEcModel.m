...
%Build the per-gene enzyme/MW/sequence arrays in two passes: (a)
%UniProt-matched, (b) KEGG-fallback-matched. Then concatenate and sort
%alphabetically so ec.genes has a stable order across runs.
ecGenesUni  = model.genes(Lia); %Will often be duplicate of model.genes, but is done here to prevent issues when it is not.
LocbUni     = Locb(Lia);
