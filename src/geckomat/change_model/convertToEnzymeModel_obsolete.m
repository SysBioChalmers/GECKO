...
function eModel = convertToEnzymeModel(irrevModel,Genes,uniprots,kcats,parameters)
    ...
    if x > 0
        ...
        for j = 1:n
            if ~isempty(uniprots{i,j}) && kcats(i,j) > 0 % if kcat=0 no mets will be added
                ...
                eModel = addEnzymesToRxn(eModel,kvalues,rxnID,newMets,{newID,newName},parameters,grRule);
            end
        end
