function [f,count] = measureAbundance(enzymes,path)
% measureAbundance
%
...
fileName = [path '/data/prot_abundance.txt'];
if exist(fileName,'file')~= 0
    ...
    fileName = [path '/data/relative_proteomics.txt'];
    if exist(fileName,'file')~= 0
        fID       = fopen(fileName);
...
if ~isempty(genes)
    ...
    %Load swissprot data:
    data      = load([path '/data/ProtDatabase.mat']);
