 function [model,enzUsages,modifications,GAM,massCoverage] = constrainEnzymes(model,parameters,f,GAM,Ptot,pIDs,data,gRate,c_UptakeExp)
% constrainEnzymes
%
...
if sum(strcmp(model.rxns,'prot_pool_exchange')) == 0
    model = constrainPool(model,~measured,full(fs*Pbase),parameters);
end
fprintf(' Done!\n')
if sum(data)==0
