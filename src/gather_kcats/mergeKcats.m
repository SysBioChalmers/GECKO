function mergedKcatList = mergeKcats(kcatLists, sourcePriority, varargin)
% mergeKcats  Merge any number of kcatLists, keeping the highest-priority
% source for each reaction.
%
% Generalizes mergeDLKcatAndFuzzyKcats: instead of two fixed inputs (DLKcat
% + fuzzy BRENDA) it accepts a cell array of kcatLists, each of which may
% itself mix several sources (for example the output of
% fetchOpenKineticsPredictor, carrying BRENDA, Sabio-RK and CataPro values
% in its kcatSource field). A caller-supplied sourcePriority decides which
% source wins for each reaction.
%
% Parameters
% ----------
% kcatLists : cell
%     cell array of kcatList structs (as produced by fuzzyKcatMatching,
%     readDLKcatOutput, fetchOpenKineticsPredictor, ...).
% sourcePriority : cell
%     cell array of tier tokens / source labels, best first. Three
%     reserved tier tokens may be used instead of a literal source label:
%
%     - 'database_exact'  an exact experimental-database measurement
%       (a BRENDA/Sabio-RK row with no fuzzy metadata, i.e. a direct OKP
%       hit, no wildcards).
%     - 'database_top'    a fuzzy BRENDA match with wildcardLvl==0 and
%       origin<=topOriginLimit.
%     - 'database_bottom' weaker fuzzy BRENDA matches within
%       wildcardLimit / bottomOriginLimit.
%
%     Any other token is matched against each row's source after folding
%     to lowercase snake_case (e.g. 'dlkcat', 'catapro'). Sources not
%     listed are dropped.
%
% Name-Value Arguments
% --------------------
% topOriginLimit : double
%     origin limit for database_top (default 6).
% bottomOriginLimit : double
%     origin limit for database_bottom (default 6).
% wildcardLimit : double
%     maximum wildcards for database_bottom's wildcard branch (default 3).
% databaseSources : cell
%     source labels treated as experimental databases, i.e. routed to the
%     database_* tiers (default {'brenda','sabio_rk'}).
%
% Returns
% -------
...
%     mergedKcatList = mergeKcats({kcatListFuzzy, kcatListOKP}, ...
%         {'database_exact', 'database_top', 'database_bottom'}, 'wildcardLimit', 2);
%
% See also
% --------
% mergeDLKcatAndFuzzyKcats, readDLKcatOutput, fuzzyKcatMatching, assignKcatValues
...
databaseSources   = p.databaseSources;
...
if ~isempty(droppedTokens)
    warning('Dropping rows from source(s) not listed in sourcePriority: %s', ...
